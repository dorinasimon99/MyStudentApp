package com.amplifyframework.datastore.generated.model;


import java.util.List;
import java.util.UUID;
import java.util.Objects;

import androidx.core.util.ObjectsCompat;

import com.amplifyframework.core.model.AuthStrategy;
import com.amplifyframework.core.model.Model;
import com.amplifyframework.core.model.ModelOperation;
import com.amplifyframework.core.model.annotations.AuthRule;
import com.amplifyframework.core.model.annotations.Index;
import com.amplifyframework.core.model.annotations.ModelConfig;
import com.amplifyframework.core.model.annotations.ModelField;
import com.amplifyframework.core.model.query.predicate.QueryField;

import static com.amplifyframework.core.model.query.predicate.QueryField.field;

/** This is an auto generated class representing the CommentData type in your schema. */
@SuppressWarnings("all")
@ModelConfig(pluralName = "CommentData", authRules = {
  @AuthRule(allow = AuthStrategy.OWNER, ownerField = "owner", identityClaim = "cognito:username", operations = { ModelOperation.CREATE, ModelOperation.UPDATE, ModelOperation.DELETE })
})
public final class CommentData implements Model {
  public static final QueryField ID = field("id");
  public static final QueryField NAME = field("name");
  public static final QueryField COMMENT = field("comment");
  public static final QueryField COURSENAME = field("coursename");
  public static final QueryField TEACHERNAME = field("teachername");
  private final @ModelField(targetType="ID", isRequired = true) String id;
  private final @ModelField(targetType="String") String name;
  private final @ModelField(targetType="String") String comment;
  private final @ModelField(targetType="String") String coursename;
  private final @ModelField(targetType="String", isRequired = true) String teachername;
  public String getId() {
      return id;
  }
  
  public String getName() {
      return name;
  }
  
  public String getComment() {
      return comment;
  }
  
  public String getCoursename() {
      return coursename;
  }
  
  public String getTeachername() {
      return teachername;
  }
  
  private CommentData(String id, String name, String comment, String coursename, String teachername) {
    this.id = id;
    this.name = name;
    this.comment = comment;
    this.coursename = coursename;
    this.teachername = teachername;
  }
  
  @Override
   public boolean equals(Object obj) {
      if (this == obj) {
        return true;
      } else if(obj == null || getClass() != obj.getClass()) {
        return false;
      } else {
      CommentData commentData = (CommentData) obj;
      return ObjectsCompat.equals(getId(), commentData.getId()) &&
              ObjectsCompat.equals(getName(), commentData.getName()) &&
              ObjectsCompat.equals(getComment(), commentData.getComment()) &&
              ObjectsCompat.equals(getCoursename(), commentData.getCoursename()) &&
              ObjectsCompat.equals(getTeachername(), commentData.getTeachername());
      }
  }
  
  @Override
   public int hashCode() {
    return new StringBuilder()
      .append(getId())
      .append(getName())
      .append(getComment())
      .append(getCoursename())
      .append(getTeachername())
      .toString()
      .hashCode();
  }
  
  @Override
   public String toString() {
    return new StringBuilder()
      .append("CommentData {")
      .append("id=" + String.valueOf(getId()) + ", ")
      .append("name=" + String.valueOf(getName()) + ", ")
      .append("comment=" + String.valueOf(getComment()) + ", ")
      .append("coursename=" + String.valueOf(getCoursename()) + ", ")
      .append("teachername=" + String.valueOf(getTeachername()))
      .append("}")
      .toString();
  }
  
  public static TeachernameStep builder() {
      return new Builder();
  }
  
  /** 
   * WARNING: This method should not be used to build an instance of this object for a CREATE mutation.
   * This is a convenience method to return an instance of the object with only its ID populated
   * to be used in the context of a parameter in a delete mutation or referencing a foreign key
   * in a relationship.
   * @param id the id of the existing item this instance will represent
   * @return an instance of this model with only ID populated
   * @throws IllegalArgumentException Checks that ID is in the proper format
   */
  public static CommentData justId(String id) {
    try {
      UUID.fromString(id); // Check that ID is in the UUID format - if not an exception is thrown
    } catch (Exception exception) {
      throw new IllegalArgumentException(
              "Model IDs must be unique in the format of UUID. This method is for creating instances " +
              "of an existing object with only its ID field for sending as a mutation parameter. When " +
              "creating a new object, use the standard builder method and leave the ID field blank."
      );
    }
    return new CommentData(
      id,
      null,
      null,
      null,
      null
    );
  }
  
  public CopyOfBuilder copyOfBuilder() {
    return new CopyOfBuilder(id,
      name,
      comment,
      coursename,
      teachername);
  }
  public interface TeachernameStep {
    BuildStep teachername(String teachername);
  }
  

  public interface BuildStep {
    CommentData build();
    BuildStep id(String id) throws IllegalArgumentException;
    BuildStep name(String name);
    BuildStep comment(String comment);
    BuildStep coursename(String coursename);
  }
  

  public static class Builder implements TeachernameStep, BuildStep {
    private String id;
    private String teachername;
    private String name;
    private String comment;
    private String coursename;
    @Override
     public CommentData build() {
        String id = this.id != null ? this.id : UUID.randomUUID().toString();
        
        return new CommentData(
          id,
          name,
          comment,
          coursename,
          teachername);
    }
    
    @Override
     public BuildStep teachername(String teachername) {
        Objects.requireNonNull(teachername);
        this.teachername = teachername;
        return this;
    }
    
    @Override
     public BuildStep name(String name) {
        this.name = name;
        return this;
    }
    
    @Override
     public BuildStep comment(String comment) {
        this.comment = comment;
        return this;
    }
    
    @Override
     public BuildStep coursename(String coursename) {
        this.coursename = coursename;
        return this;
    }
    
    /** 
     * WARNING: Do not set ID when creating a new object. Leave this blank and one will be auto generated for you.
     * This should only be set when referring to an already existing object.
     * @param id id
     * @return Current Builder instance, for fluent method chaining
     * @throws IllegalArgumentException Checks that ID is in the proper format
     */
    public BuildStep id(String id) throws IllegalArgumentException {
        this.id = id;
        
        try {
            UUID.fromString(id); // Check that ID is in the UUID format - if not an exception is thrown
        } catch (Exception exception) {
          throw new IllegalArgumentException("Model IDs must be unique in the format of UUID.",
                    exception);
        }
        
        return this;
    }
  }
  

  public final class CopyOfBuilder extends Builder {
    private CopyOfBuilder(String id, String name, String comment, String coursename, String teachername) {
      super.id(id);
      super.teachername(teachername)
        .name(name)
        .comment(comment)
        .coursename(coursename);
    }
    
    @Override
     public CopyOfBuilder teachername(String teachername) {
      return (CopyOfBuilder) super.teachername(teachername);
    }
    
    @Override
     public CopyOfBuilder name(String name) {
      return (CopyOfBuilder) super.name(name);
    }
    
    @Override
     public CopyOfBuilder comment(String comment) {
      return (CopyOfBuilder) super.comment(comment);
    }
    
    @Override
     public CopyOfBuilder coursename(String coursename) {
      return (CopyOfBuilder) super.coursename(coursename);
    }
  }
  
}
